...
class CorrectMessageProof {
   ...
   public:
    ...
    CorrectMessageProof(const std::vector<BigNum>& eVec,
                        const std::vector<BigNum>& zVec,
                        const std::vector<BigNum>& aVec,
                        const BigNum& cipher,
                        const std::vector<BigNum>& validMsgs,
                        const BigNum& n)
        : e_vec(eVec), z_vec(zVec), a_vec(aVec), ciphertext(cipher), valid_messages(validMsgs), n(n), nn(n * n) {}

    CorrectMessageProof(const BigNum& r,
                        const std::vector<BigNum>& ziVec,
                        const std::vector<BigNum>& eiVec,
                        const BigNum& w,
                        const BigNum& encrypted_mesg,
                        size_t index,
                        const std::vector<BigNum>& validMsgs,
        ...
        for (const auto& m : valid_messages) {
            BigNum gm = (n + BigNum(1)).modExp(m, nn);
    ...
    static CorrectMessageProof prove(const BigNum& n,
                                     const std::vector<BigNum>& validMessages,
